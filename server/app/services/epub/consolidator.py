...
class Consolidator:
        ...
        for file_name in self.order:
            ...
            if full_file_path.is_file():
                with open(
                    ...
                    cleaned_content = self.remove_html_tags_and_empty_lines(
                        file_content
                    )
                    ...
                    if any(
                        ...
                        print(
                            f"File {file_name} detected as a copyright page, skipping."
                        )
                        continue
                    ...
                    if (
                        ...
                        print(
                            f"File {file_name} seems to be an index or footnote, skipping."
                        )
                        continue
    ...
    def save_consolidated_files(self, combined_files):
        ...
        output_file = output_file_base.with_name(
            f"{output_file_base.stem}_{file_number}.txt"
        )
        char_count = 0
        ...
        print(
            f"Files consolidated and saved to {output_file_base.stem}_*.txt files successfully."
        )
...
if __name__ == "__main__":
    ...
    for book_folder in books_folder.iterdir():
        if book_folder.is_dir():
            ...
            consolidator = Consolidator(book_folder, 9999999999)
